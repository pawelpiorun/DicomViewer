...
BinaryThinningImageFilter3D::BinaryThinningImageFilter3D()
{
...
DicomSeriesType::Pointer BinaryThinningImageFilter3D::GetThinning(void)
{
	return  dynamic_cast<OutputImageType *>(
		this->ProcessObject::GetOutput(0));
}

void BinaryThinningImageFilter3D::PrepareData(void)
{
	DicomSeriesType::Pointer thinImage = GetThinning();

	auto inputImage =
		dynamic_cast<const DicomSeriesType  *>(ProcessObject::GetInput(0));
	...
	itk::ImageRegionConstIterator<DicomSeriesType>  it(inputImage, region);
	itk::ImageRegionIterator<DicomSeriesType> ot(thinImage, region);
	...
	while (!ot.IsAtEnd())
	{
		if (it.Get())
		{
			ot.Set(_foregroundValue);
		}
		else
		{
			ot.Set(_backgroundValue);
		}
		++it;
		++ot;
	}
...
void BinaryThinningImageFilter3D::ComputeThinImage()
{
	auto thinImage = GetThinning();

	typename DicomSeriesType::RegionType region = thinImage->GetRequestedRegion();
	...
	while (unchangedBorders < 6)  // loop until no change for all the six border types
	{
		...
		{
			//this->UpdateProgress(this->GetProgress() + 0.001); // fake progress
...
void BinaryThinningImageFilter3D::GenerateData()
{
	...
	std::cout << "[Skeletonize] Skeletonization done." << std::endl;
}

void BinaryThinningImageFilter3D::fillEulerLUT(int *LUT)
{
	LUT[1] = 1;
	LUT[3] = -1;
	LUT[5] = -1;
...
bool BinaryThinningImageFilter3D::isEulerInvariant(NeighborhoodType neighbors, int *LUT)
{
...
bool BinaryThinningImageFilter3D::isSimplePoint(NeighborhoodType neighbors)
{
...
void BinaryThinningImageFilter3D::Octree_labeling(int octant, int label, int *cube)
{
